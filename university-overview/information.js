...
(function (root, factory) {
  ...
  function build(formula, subjective, review) {
    ...
    const items = [];
    if (review?.withholdTotal) items.push({ label: '전형 총점', value: review.withholdReason || '환산 단위 확인 중' });
    else if (total != null) items.push({ label: '전형 총점', value: `${total}점` });
    if (practical != null && practical > 0) items.push({ label: '실기 만점(등록 기준)', value: review?.practicalDisplay || `${practical}점` });
    if (policy) {
    ...
    if (policy) {
      ...
      if (policy.choices.length) notes.push(`${policy.selectionLabel}: ${policy.choices.join('·')} 중 선택. 개인상담과 통합계산기에서 선택할 수 있습니다.`);
