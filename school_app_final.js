...
(function () {
  ...
  const finalResults = window.MaxFinalApplicantResults;
  privacy.init();
  ...
  function renderCards() {
    ...
    const anomalySet = finalResults.detectAnomalySet(sorted);
    ...
    const html = sorted.map((a, idx) => {
      ...
      const resultChips = [
        finalResults.resultBadge('1단계', a.result_1st, window.escapeHtml),
        finalResults.resultBadge('최초', a.result_first, window.escapeHtml),
        finalResults.resultBadge('최종', a.result_final, window.escapeHtml),
      ...
      return `
        <div class="applicant-card ${isMine ? 'is-mine' : ''} ${anomaly ? 'has-anomaly anomaly-' + anomaly.level : ''}">
          <div class="card-header">
            ...
            <span class="name">${esc(visible.name || '-')}</span>
            <span class="branch">${esc(visible.branch || '-')}</span>
            ...
            <span class="school">${esc(visible.school_name || '')}</span>
          </div>
