...
const pdfRender = require('./pdfRender.js');
const app = express();
const port = 9090;
...
const db = mysql.createPool({ host: '127.0.0.1', user: 'maxilsan', password: 'q141171616!', database: 'jungsi', charset: 'utf8mb4', waitForConnections: true, connectionLimit: 10, queueLimit: 0 });
const {
...
app.post('/jungsi/students/scores/recompute', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    let where = "s.학년도=? AND s.모형=? AND s.입력유형='raw'";
    const params = [year, exam_type];
    ...
    for (const r of rows) {
      ...
      await conn.query(
        `UPDATE \`학생수능성적\` SET
         ...
         WHERE student_id=? AND 학년도=? AND 모형=?`,
        [
...
app.post('/jungsi/score-config/set-bulk', authMiddleware, async (req, res) => {
  const { year, items, exam } = req.body;
  const examType = exam || '수능';
...
app.post('/jungsi/student/score/set-wide', authMiddleware, async (req, res) => {
    ...
    try {
     ...
     // 5. [학생수능성적] 테이블 저장 (UPSERT - 컬럼명/업데이트 구문 명시)
        ...
        const params = [
            savedData.student_id, savedData.학년도, examType, savedData.입력유형,
...
app.post('/jungsi/students/scores/bulk-set-wide', authMiddleware, async (req, res) => {
    ...
    const { 학년도, 입력유형, studentScores, 모형: reqExam } = req.body; // 프론트에서 보낸 데이터
    const examType = reqExam || '수능'; // 기본값 '수능' (하위호환)
...
app.post('/jungsi/scores/list', authMiddleware, async (req, res) => {
  const { year, student_ids, exam } = req.body;
  const examType = exam || '수능';
  ...
  try {
    ...
    for (const raw of items) {
      ...
      await conn.query(sql, [
        x.student_id, year, examType, x.입력유형,
...
app.get('/jungsi/cutoffs/:year', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const cutoffSql = `
            SELECT U_ID, branch_name, 수능컷, 총점컷, \`25년총점컷\`, \`26년총점컷\`
            FROM 정시_컷점수
...
app.post('/jungsi/cutoffs/set', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const upsertSql = `
            INSERT INTO 정시_컷점수 (학년도, U_ID, branch_name, 수능컷, 총점컷, \`25년총점컷\`, \`26년총점컷\`)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
                수능컷 = VALUES(수능컷),
                총점컷 = VALUES(총점컷),
                \`25년총점컷\` = VALUES(\`25년총점컷\`),
                \`26년총점컷\` = VALUES(\`26년총점컷\`),
                updated_at = NOW()
        `;
        ...
        for (const item of updates) {
            ...
            // Admin 역할 처리
            if (isAdmin(req.user) && (item.맥스_수능컷 !== undefined || item.맥스_총점컷 !== undefined || item['25년총점컷'] !== undefined || item['26년총점컷'] !== undefined)) {
                ...
                const total26 = item['26년총점컷'] === '' ? null : item['26년총점컷'];
                 ...
                 if ((maxSuneung === null || !isNaN(parseFloat(maxSuneung))) &&
                     ...
                     (total26 === null || !isNaN(parseFloat(total26))))
                 {
                    console.log(` -> Admin updating MAX for U_ID ${U_ID}: 수능=${maxSuneung}, 총점=${maxTotal}, 25총점=${total25}, 26총점=${total26}`);
                    await connection.query(upsertSql, [year, U_ID, 'MAX', maxSuneung, maxTotal, total25, total26]);
                    updatedMaxCount++;
                 } else {
             ...
             if (item.지점_수능컷 !== undefined || item.지점_총점컷 !== undefined) {
                     ...
                     (branchTotal === null || !isNaN(parseFloat(branchTotal))))
                 {
                     ...
                     await connection.query(upsertSql, [year, U_ID, branch, branchSuneung, branchTotal, null, null]); // 25/26년총점컷은 null
                     updatedBranchCount++;
                 } else {
...
if (입력유형 === 'raw') { // (가채점)
        ...
        `;
        const params = [
            savedData.student_id, savedData.학년도, examType || '수능', savedData.입력유형,
            savedData.국어_선택과목, savedData.국어_원점수, savedData.국어_표준점수, savedData.국어_백분위, savedData.국어_등급,
            savedData.수학_선택과목, savedData.수학_원점수, savedData.수학_표준점수, savedData.수학_백분위, savedData.수학_등급,
            savedData.영어_원점수, savedData.영어_등급,
            savedData.한국사_원점수, savedData.한국사_등급,
...
// ============ PDF 상담지 렌더 — Puppeteer (2026-04-18 추가) ============
// POST /jungsi/counseling/render-pdf
// Body: { html: '<complete HTML>', filename?: '...' }
// Response: application/pdf binary stream
app.post('/jungsi/counseling/render-pdf', authMiddleware, async (req, res) => {
    const t0 = Date.now();
    try {
        const { html, filename } = req.body || {};
        if (!html || typeof html !== 'string') {
            return res.status(400).json({ success: false, message: 'html 필드 필요' });
        }
        if (html.length > 5_000_000) {
            return res.status(413).json({ success: false, message: 'HTML too large (>5MB)' });
        }
        const pdf = await pdfRender.renderPdfFromHtml(html, {
            format: 'A4',
            landscape: true,
            printBackground: true,
            margin: { top: '0mm', right: '0mm', bottom: '0mm', left: '0mm' },
        });
        const name = (filename || 'counsel').replace(/[^\w가-힣.-]/g, '_');
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', 'attachment; filename="' + encodeURIComponent(name) + '.pdf"');
        res.setHeader('Content-Length', pdf.length);
        console.log('[render-pdf] user=' + req.user.userid + ' bytes=' + pdf.length + ' ms=' + (Date.now()-t0));
        return res.end(pdf);
    } catch (err) {
        console.error('[render-pdf] ERROR', err.message);
        return res.status(500).json({ success: false, message: 'PDF 생성 실패: ' + err.message });
    }
});


// ============ 모평 대비 트렌드 (2026-04-18 추가) ============
// GET /jungsi/counseling/trend/:student_id/:year/:exam
// 현재 선택 모형과 시간적으로 직전 모형(있으면)을 반환
app.get('/jungsi/counseling/trend/:student_id/:year/:exam', authMiddleware, async (req, res) => {
    const { student_id, year, exam } = req.params;
    const { branch } = req.user;
    try {
        ...
        if (!own.length) return res.status(403).json({ success: false, message: '권한 없음' });

        // 모형 순서: 최신 → 이전
        const ORDER = ['수능', '11월', '9월', '6월', '3월'];
        const curIdx = ORDER.indexOf(exam);
        if (curIdx < 0 || curIdx >= ORDER.length - 1) {
            return res.json({ success: true, hasPrev: false });
        }
        const candidates = ORDER.slice(curIdx + 1);

        const placeholders = candidates.map(() => '?').join(',');
        const [prevRows] = await db.query(
            `SELECT * FROM 학생수능성적
             WHERE student_id=? AND 학년도=? AND 모형 IN (${placeholders})
             ORDER BY FIELD(모형, ${placeholders}) LIMIT 1`,
            [student_id, year, ...candidates, ...candidates]
        );
        if (!prevRows.length) {
            return res.json({ success: true, hasPrev: false });
        }
        ...
        const [curRows] = await db.query(
            'SELECT * FROM 학생수능성적 WHERE student_id=? AND 학년도=? AND 모형=?',
            [student_id, year, exam]
        );
...
// ─────────────────────────────────────────────────
// 실기 배점표 CRUD (silgi-editor 관리자 페이지)
// 추가일: 2026-04-20 · 테이블: 정시실기배점
// ─────────────────────────────────────────────────
app.get('/jungsi/practical-scores/:uid/:year', authMiddleware, async (req, res) => {
    try {
        const { uid, year } = req.params;
        const [rows] = await db.query(
            "SELECT id, U_ID, 학년도, 종목명, 성별, 기록, 배점 FROM `정시실기배점` WHERE U_ID = ? AND 학년도 = ? ORDER BY 종목명, 성별, id",
            [uid, year]
        );
        res.json({ success: true, scores: rows });
    } catch (err) {
        console.error('[practical-scores/get]', err);
        res.status(500).json({ success: false, message: 'DB 오류: ' + err.message });
    }
});

app.post('/jungsi/practical-scores/save', authMiddleware, async (req, res) => {
    try {
        const { U_ID, 학년도, 종목명, 성별, 기록, 배점 } = req.body || {};
        if (!U_ID || !학년도 || !종목명 || 기록 == null || 배점 == null) {
            return res.status(400).json({ success: false, message: '필수 필드 누락' });
        }
        const [result] = await db.query(
            "INSERT INTO `정시실기배점` (U_ID, 학년도, 종목명, 성별, 기록, 배점) VALUES (?, ?, ?, ?, ?, ?)",
            [U_ID, String(학년도), 종목명, 성별 || null, String(기록), String(배점)]
        );
        res.json({ success: true, id: result.insertId, message: '저장 완료' });
    } catch (err) {
        console.error('[practical-scores/save]', err);
        res.status(500).json({ success: false, message: 'DB 오류: ' + err.message });
    }
});

app.post('/jungsi/practical-scores/bulk-save', authMiddleware, async (req, res) => {
    try {
        const { U_ID, year, eventName, gender, rows_text } = req.body || {};
        if (!U_ID || !year || !eventName || !rows_text) {
            return res.status(400).json({ success: false, message: '필수 필드 누락' });
        }
        const pairs = String(rows_text).split('\n')
            .map(l => l.split('\t'))
            .filter(p => p.length === 2 && p[0].trim());
        if (pairs.length === 0) {
            return res.status(400).json({ success: false, message: '파싱된 행 없음' });
        }
        const values = pairs.map(([rec, sc]) => [U_ID, String(year), eventName, gender || null, rec.trim(), sc.trim()]);
        await db.query(
            "INSERT INTO `정시실기배점` (U_ID, 학년도, 종목명, 성별, 기록, 배점) VALUES ?",
            [values]
        );
        res.json({ success: true, message: `${pairs.length}건 저장 완료`, count: pairs.length });
    } catch (err) {
        console.error('[practical-scores/bulk-save]', err);
        res.status(500).json({ success: false, message: 'DB 오류: ' + err.message });
    }
});

app.delete('/jungsi/practical-scores/delete/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const [result] = await db.query("DELETE FROM `정시실기배점` WHERE id = ?", [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ success: false, message: '해당 ID 없음' });
        }
        res.json({ success: true, message: '삭제 완료' });
    } catch (err) {
        console.error('[practical-scores/delete]', err);
        res.status(500).json({ success: false, message: 'DB 오류: ' + err.message });
    }
});
...
// ─────────────────────────────────────────────────
// 영어/한국사 등급점수 편집 (scores_editor 관리자 페이지)
// 추가일: 2026-04-20 · 테이블: 정시반영비율 (english_scores, history_scores JSON)
// ─────────────────────────────────────────────────
app.get('/jungsi/scores/list/:year', authMiddleware, async (req, res) => {
    try {
        const { year } = req.params;
        const [rows] = await db.query(
            `SELECT b.U_ID, b.대학명, b.학과명, b.군,
                    r.english_scores, r.history_scores
             FROM \`정시기본\` AS b
             LEFT JOIN \`정시반영비율\` AS r
...
app.post('/jungsi/scores/bulk-save', authMiddleware, async (req, res) => {
    try {
        const { year, scores_data } = req.body || {};
        if (!year || !Array.isArray(scores_data)) {
            return res.status(400).json({ success: false, message: '필수 필드 누락' });
        }
        ...
        const textToJson = (text) => {
            ...
            for (let i = 0; i < 9; i++) {
                const v = parts[i];
                if (v !== undefined && String(v).trim() !== '') obj[String(i + 1)] = String(v).trim();
            }
            return Object.keys(obj).length ? JSON.stringify(obj) : null;
        };

        let updated = 0, inserted = 0, skipped = 0;
        for (const row of scores_data) {
            const uid = row.U_ID;
            if (!uid) { skipped++; continue; }
            const enJson = textToJson(row.english_text);
            const hiJson = textToJson(row.history_text);

            const [upd] = await db.query(
                'UPDATE `정시반영비율` SET english_scores = ?, history_scores = ? WHERE U_ID = ? AND 학년도 = ?',
                [enJson, hiJson, uid, year]
            );
            if (upd.affectedRows > 0) {
                updated++;
            } else {
                await db.query(
                    'INSERT INTO `정시반영비율` (U_ID, 학년도, english_scores, history_scores) VALUES (?, ?, ?, ?)',
                    [uid, year, enJson, hiJson]
                );
                inserted++;
            }
        }
        ...
        res.json({
            success: true,
            message: `저장 완료 — 업데이트 ${updated}, 신규 ${inserted}` + (skipped ? `, 스킵 ${skipped}` : ''),
            updated, inserted, skipped
        });
    } catch (err) {
        console.error('[scores/bulk-save]', err);
        res.status(500).json({ success: false, message: 'DB 오류: ' + err.message });
    }
});
...
// ─────────────────────────────────────────────────
// 학교별 상담 (v4) — 상담학생 + 실기기록 + 컷 + 대학 실기배점표
// GET /jungsi/counseling/by-university/:U_ID/:year
// 작성: 2026-04-20
// ─────────────────────────────────────────────────
app.get('/jungsi/counseling/by-university/:U_ID/:year', authMiddleware, async (req, res) => {
    try {
        ...
        const applicantsSql = `
            SELECT
              c.\`상담목록_ID\`  AS counsel_id,
              c.\`학생_ID\`      AS student_id,
              ...
              c.\`대학학과_ID\`   AS U_ID,
              c.\`상담_수능점수\`        AS suneung_score,
              c.\`상담_내신점수\`        AS naesin_score,
              c.\`상담_실기반영점수\`     AS silgi_score,
              c.\`상담_계산총점\`        AS total_score,
              c.\`상담_실기기록\`        AS silgi_record,
              c.\`메모\`,
              c.\`생성일시\`,
              c.\`수정일시\`,
              b.\`대학명\` AS university,
        ...
        `;
        const cutsSql = `
            SELECT branch_name, \`수능컷\`, \`총점컷\`, \`25년총점컷\`, \`26년총점컷\`
            FROM \`정시_컷점수\`
            WHERE U_ID = ? AND \`학년도\` = ? AND branch_name IN (?, 'MAX')
        `;
        const scoreTableSql = `
            SELECT id, \`종목명\`, \`성별\`, \`기록\`, \`배점\`
            FROM \`정시실기배점\`
            WHERE U_ID = ? AND \`학년도\` = ?
            ORDER BY \`종목명\`, \`성별\`, CAST(\`기록\` AS DECIMAL(10,3))
        `;
        ...
        const cuts = { mine: null, max: null };
        for (const row of cutRows) {
            if (row.branch_name === 'MAX') cuts.max = row;
...
// ─────────────────────────────────────────────────
// 학교별 상담 — 개별 학생 실기 기록/점수/메모 업데이트 (+ 총점 재계산)
// PUT /jungsi/counseling/update/:counsel_id
// body: { silgi_record, silgi_score, memo }
// 서버가 수능+내신+실기 합산하여 상담_계산총점 도 갱신
// ─────────────────────────────────────────────────
app.put('/jungsi/counseling/update/:counsel_id', authMiddleware, async (req, res) => {
    try {
        const branch = req.user && req.user.branch;
        if (!branch) return res.status(400).json({ success: false, message: 'branch 정보 없음' });
        const { counsel_id } = req.params;
        if (!counsel_id) return res.status(400).json({ success: false, message: 'counsel_id 필요' });

        const { silgi_record, silgi_score, memo } = req.body || {};

        // 소유자 확인 + 기존 수능/내신 점수 조회 (총점 재계산용)
        const [[row]] = await db.query(
            `SELECT c.\`상담목록_ID\`,
                    c.\`상담_수능점수\` AS suneung,
                    c.\`상담_내신점수\` AS naesin,
                    s.branch_name
             FROM \`정시_상담목록\` AS c
             JOIN \`학생기본정보\`  AS s ON c.\`학생_ID\` = s.student_id
        ...
        if (!row) return res.status(404).json({ success: false, message: '상담 기록 없음' });
        if (row.branch_name !== branch) {
            return res.status(403).json({ success: false, message: '다른 지점 학생은 수정 불가' });
        }

        const fields = [];
        const params = [];
        let newSilgiNum = null;
        if (silgi_record !== undefined) {
            const json = (silgi_record === null || silgi_record === '')
                ? null
                : (typeof silgi_record === 'string' ? silgi_record : JSON.stringify(silgi_record));
            fields.push('`상담_실기기록` = ?');
            params.push(json);
        }
        if (silgi_score !== undefined) {
            const num = (silgi_score === null || silgi_score === '') ? null : Number(silgi_score);
            newSilgiNum = Number.isFinite(num) ? num : null;
            fields.push('`상담_실기반영점수` = ?');
            params.push(newSilgiNum);
        }
        if (memo !== undefined) {
            fields.push('`메모` = ?');
        ...
        // 총점 재계산: 수능 + 내신 + 실기 (단순 합, counsel.new.html 패턴)
        if (silgi_score !== undefined) {
            const suneungNum = Number(row.suneung) || 0;
            const naesinNum = Number(row.naesin) || 0;
            const silgiNum = newSilgiNum != null ? newSilgiNum : 0;
            const newTotal = suneungNum + naesinNum + silgiNum;
            fields.push('`상담_계산총점` = ?');
            params.push(newTotal);
        }

        if (fields.length === 0) {
            return res.status(400).json({ success: false, message: '업데이트할 필드 없음' });
        }

        params.push(counsel_id);
        const [result] = await db.query(
            `UPDATE \`정시_상담목록\` SET ${fields.join(', ')} WHERE \`상담목록_ID\` = ?`,
            params
        );

        res.json({ success: true, affected: result.affectedRows });
    } catch (err) {
        console.error('[/jungsi/counseling/update]', err);
        res.status(500).json({ success: false, message: 'DB 오류: ' + err.message });
    }
});
...
// ─────────────────────────────────────────────────
// 학교별 상담 — 우리 지점이 실제로 상담 저장한 대학/학과 목록
// GET /jungsi/counseling/saved-universes/:year
// 추가일: 2026-04-20
// ─────────────────────────────────────────────────
app.get('/jungsi/counseling/saved-universes/:year', authMiddleware, async (req, res) => {
    try {
        const branch = req.user && req.user.branch;
        if (!branch) return res.status(400).json({ success: false, message: 'branch 정보 없음' });
        const { year } = req.params;
