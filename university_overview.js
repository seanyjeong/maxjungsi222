  ...
  function mergeRows(schools, cutoffs, filter) {
    ...
    return schools.map(s => {
      ...
      return {
        ...
        stageComparison: review?.stageComparison || '',
  ...
  function render(rows) {
    ...
    tbody.innerHTML = rows.map(r => `
      <tr data-uid="${r.U_ID}">
        ...
        <td class="col-univ uni-dept"><button class="school-detail-link" type="button"><span class="univ-name">${window.escapeHtml ? window.escapeHtml(r.univ) : r.univ}${(window.renderSchoolTags && window.renderSchoolTags(r.tags)) || ''}</span><span class="dept-name">${window.escapeHtml ? window.escapeHtml(r.dept) : r.dept}</span>${r.reviewStatus ? `<span class="review-summary">2027 반영: ${window.escapeHtml(r.reviewStatus)}</span>` : ''}</button></td>
        ...
        <td class="ratio">${formatDangye(r.단계별)}${r.stageComparison ? '<span class="review-summary">전년과 동일</span>' : ''}</td>
