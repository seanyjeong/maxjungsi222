...
def test_stage_one_overview_coefficients_and_legacy_isolation(browser, base_url):
    ...
    expect(row.locator('.review-summary').last).to_have_text('전년과 동일')
    expect(row).to_contain_text('표준점수 × 1.2')
    row.click()
    expect(page.locator('#mdScoreInfo')).to_contain_text('1단계 수능 점수')
    expect(page.locator('#mdScoreInfo')).to_contain_text('2026학년도 3배수 → 2027학년도 3배수')
    ...
    expect(row.locator('.review-summary')).to_have_count(0)
    row.click()
